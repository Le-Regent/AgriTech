...
import { getWeatherData, getCurrentPosition, getForecastData, WeatherData, ForecastData } from '../src/lib/weatherService';
import { supabaseService } from '../src/services/supabaseService';
import { CropDiagnosis } from '../src/types';
import { downloadDiagnosisReport } from '../src/lib/diagnosisUtils';
    ...
    async function fetchData() {
      try {
        ...
        try {
          ...
          // Default to a generic location (e.g., Douala, Cameroon)
        ...
        if (isFarmer && user?.id) {
          try {
            ...
            console.error('Failed to fetch diagnoses or products:', dbError.message || dbError);
    ...
    return (
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
          {[
            { label: 'Active Orders', value: '3', icon: 'shopping_bag', color: 'text-blue-500', bg: 'bg-blue-50 dark:bg-blue-500/10' },
            { label: 'Saved Items', value: '12', icon: 'favorite', color: 'text-red-500', bg: 'bg-red-50 dark:bg-red-500/10' },
            { label: 'Total Spent', value: '75,000 CFA', icon: 'payments', color: 'text-green-500', bg: 'bg-green-50 dark:bg-green-500/10' },
            { label: 'Reward Points', value: '450', icon: 'stars', color: 'text-purple-500', bg: 'bg-purple-50 dark:bg-purple-500/10' },
          ].map((stat, i) => (
          ...
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
              ...
              <div className="space-y-4">
                {[
                  { id: '#ORD-1234', date: '2 days ago', status: 'Delivered', total: '25,000 CFA', items: 'White Cassava x 50kg' },
                  { id: '#ORD-1235', date: '5 days ago', status: 'In Transit', total: '15,000 CFA', items: 'Sweet Plantains x 5 bunches' },
                ].map((order, i) => (
                  <div key={i} className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-800 rounded-xl transition-colors">
                    <div>
                      <h4 className="font-bold text-sm dark:text-white">{order.id}</h4>
                    ...
                    <div className="text-right">
                      <p className="font-black text-sm dark:text-white">{order.total}</p>
          ...
          <div className="space-y-6 sm:space-y-8">
            ...
            <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[1.5rem] sm:rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { label: 'Reorder', icon: 'reorder' },
                  { label: 'Support', icon: 'support_agent' },
                  { label: 'Coupons', icon: 'confirmation_number' },
                  { label: 'Settings', icon: 'settings' },
                ].map((action, i) => (
                  <button key={i} className="flex flex-col items-center gap-2 p-3 bg-slate-50 dark:bg-slate-800 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors group">
