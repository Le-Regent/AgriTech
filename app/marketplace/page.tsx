        ...
        {loading ? (
          ...
          sortedProducts.map((product, index) => (
            <Link
              ...
              className={`group bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all overflow-hidden flex flex-col ${index === 0 ? 'product-card-first' : ''}`}
            >
              <div className="aspect-[4/3] relative overflow-hidden">
                <ResponsiveImage
                  src={product.image_url || 'https://picsum.photos/seed/product/400/300'}
                  alt={`Fresh ${product.name} available at the marketplace`}
                ...
                <button className="absolute bottom-4 right-4 w-10 h-10 bg-white/90 dark:bg-slate-900/90 backdrop-blur rounded-xl shadow-lg flex items-center justify-center text-slate-400 hover:text-red-500 transition-colors">
                  <span className="material-symbols-outlined">favorite</span>
                </button>

                <button 
                  onClick={(e) => toggleProductSelection(e, product.id)}
                  className={`absolute top-4 right-4 w-10 h-10 backdrop-blur rounded-xl shadow-lg flex items-center justify-center transition-all ${index === 0 ? 'compare-btn-first' : ''} ${selectedProducts.includes(product.id) ? 'bg-indigo-600 text-white' : 'bg-white/90 dark:bg-slate-900/90 text-slate-400 hover:text-indigo-600'}`}
                ...
                {(!product.image_url || product.image_url.includes('picsum.photos')) && (
                  <button 
                    ...
                    className={`absolute bottom-4 left-4 h-10 px-3 bg-white/90 dark:bg-slate-900/90 backdrop-blur rounded-xl shadow-lg flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-all disabled:opacity-50 disabled:cursor-not-allowed ${index === 0 ? 'ai-image-btn-first' : ''}`}
              ...
              <div className="p-6 space-y-4 flex-1 flex flex-col">
                <div className="flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <h3 className="font-black text-lg group-hover:text-primary dark:text-white transition-colors">{product.name}</h3>
                    <p className="font-black text-primary">{product.price.toLocaleString()}<span className="text-xs text-slate-400 dark:text-slate-500 font-bold"> CFA/{product.unit}</span></p>
                  </div>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1 text-slate-400 dark:text-slate-500">
                      <span className="material-symbols-outlined text-[16px]">location_on</span>
                      <span className="text-xs font-bold">{product.location}</span>
                    </div>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">{product.profiles?.full_name || 'Unknown Farmer'}</span>
                  ...
                  <div className="flex -space-x-2">
                    {[1, 2, 3].map((i) => (
                      <ResponsiveImage
                        ...
                        alt={`Avatar of customer ${i}`}
                        baseWidth={40}
                        baseHeight={40}
                      />
                    ))}
                    <div className="w-8 h-8 rounded-full border-2 border-white dark:border-slate-900 bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-[10px] font-bold text-slate-400 dark:text-slate-500">
                      +12
                    </div>
                  </div>
                  <button 
                    ...
                    className="bg-primary text-white px-4 py-2 rounded-xl text-xs font-black hover:bg-primary/90 transition-all flex items-center gap-2"
                  >
