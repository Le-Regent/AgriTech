...
function CartContent() {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // 2. Create Order and Items in one call
      ...
      if (order) {
        ...
        for (const item of cart) {
          try {
            await supabaseService.updateProductStock(item.id, item.quantity);
          } catch (stockError) {
      ...
      toast.error('Checkout failed. Please try again.', { id: loadingToast });
    } finally {
