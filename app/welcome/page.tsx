...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-background-dark transition-colors duration-300">
      <nav className="h-20 px-4 sm:px-8 flex items-center justify-between border-b border-slate-100 dark:border-slate-800 transition-colors">
      ...
      <section className="py-12 sm:py-24 px-4 sm:px-8 max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center">
        <div className="space-y-6 sm:space-y-8 text-center lg:text-left">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-green-50 dark:bg-green-900/20 text-primary rounded-full mx-auto lg:mx-0 transition-colors">
            <span className="w-2 h-2 bg-primary rounded-full animate-pulse"></span>
            <span className="text-[10px] sm:text-xs font-black uppercase tracking-widest">New: AI Diagnosis v2.0</span>
          </div>
          <h2 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tighter leading-[1.1] lg:leading-[0.9] dark:text-white transition-colors">
            The Future of <span className="text-primary">Farming</span> is Here.
          </h2>
          <p className="text-lg sm:text-xl text-slate-500 dark:text-slate-400 leading-relaxed max-w-lg mx-auto lg:mx-0 transition-colors">
          ...
          <div className="flex flex-wrap items-center justify-center lg:justify-start gap-6 sm:gap-8 pt-8 border-t border-slate-100 dark:border-slate-800 transition-colors">
            <div>
        ...
        <div className="relative mt-8 lg:mt-0">
          <div className="aspect-square bg-slate-100 dark:bg-slate-800 rounded-[2rem] sm:rounded-[4rem] overflow-hidden shadow-2xl relative transition-colors">
            <ResponsiveImage 
              ...
              alt="Modern farmer using a tablet in a lush green field, symbolizing the fusion of technology and agriculture" 
          ...
          {/* Decorative cards hidden on mobile */}
          <div className="hidden sm:block absolute -bottom-10 -left-10 bg-white dark:bg-slate-900 p-6 rounded-3xl shadow-2xl border border-slate-100 dark:border-slate-800 max-w-xs space-y-4 animate-bounce-slow transition-colors">
          ...
          <div className="hidden sm:block absolute -top-10 -right-10 bg-background-dark text-white p-6 rounded-3xl shadow-2xl max-w-xs space-y-4">
            <div className="flex items-center gap-3">
        ...
        <div className="max-w-7xl mx-auto space-y-12 sm:space-y-16">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
            {[
              ...
              <div key={i} className="bg-white dark:bg-slate-900 p-8 sm:p-10 rounded-[2rem] sm:rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-all group">
