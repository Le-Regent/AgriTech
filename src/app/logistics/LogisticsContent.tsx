...
import { Truck, Package, CheckCircle2, MapPin, Gauge, Timer, TrendingUp, Search, Filter, Clock, ChevronRight, Check } from 'lucide-react';

const TRACKING_STEPS = [
  { status: 'processing', label: 'Processing', icon: Package, color: 'blue' },
  { status: 'shipped', label: 'Shipped', icon: Truck, color: 'amber' },
  { status: 'delivered', label: 'Delivered', icon: CheckCircle2, color: 'green' }
];
...
export default function LogisticsContent() {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
      {/* Filters Overlay for Mobile sticky header could go here */}
      <div className="lg:col-span-2 space-y-6">
        <div className="flex bg-slate-50 dark:bg-slate-800 p-1.5 rounded-2xl border border-slate-100 dark:border-slate-700 w-fit mb-4">
          ...
          {filteredShipments.length === 0 ? (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto font-bold font-mono text-[10px] uppercase tracking-widest leading-relaxed">No {filter === 'all' ? '' : filter} shipments at the moment.</p>
              </div>
            </motion.div>
          ) : (
                      ...
                      return (
                        <div key={step.status} className="flex flex-col items-center gap-3">
                          <div className={`w-12 h-12 rounded-2xl flex items-center justify-center z-20 transition-all duration-500 ${
                            ...
                            {isActive ? <Check size={20} /> : <step.icon size={20} />}
                          </div>
        ...
        <div className="bg-white dark:bg-surface-dark p-8 rounded-[3rem] border border-slate-100 dark:border-border-dark shadow-sm space-y-8 transition-colors">
          ...
          <div className="space-y-6">
            {[
              ...
              { label: 'transit_time', value: '1.4 Days', color: 'text-primary', icon: Clock },
              { label: 'efficiency', value: 'High', color: 'text-sky-500', icon: TrendingUp },
            ].map((stat, i) => (
