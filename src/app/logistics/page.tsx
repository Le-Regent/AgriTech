...
function LogisticsContent() {
  ...
  if (loading && !shipments.length) {
    return (
      <div className="space-y-4 sm:space-y-8 max-w-7xl mx-auto px-4 sm:px-0">
        <div className="h-24 w-full bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] animate-pulse" />
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-2 space-y-6">
            {[1, 2, 3].map(i => (
              <div key={i} className="h-64 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[2.5rem] animate-pulse" />
            ))}
          </div>
          <div className="space-y-6">
            <div className="h-96 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[3rem] animate-pulse" />
            <div className="h-64 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-[3rem] animate-pulse" />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="visible"
      className="space-y-4 sm:space-y-8 max-w-7xl mx-auto"
    >
      ...
      >
        <div className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all">
          <div className="space-y-1">
            <h2 className="text-2xl sm:text-3xl font-black tracking-tight dark:text-white flex items-center gap-3">
              <span className="material-symbols-outlined text-primary text-3xl">local_shipping</span>
              {t('logistics')}
            </h2>
            <p className="text-[10px] sm:text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest italic opacity-70">Real-time supply chain monitoring 🇨🇲</p>
          </div>
          
          <div className="flex bg-slate-50 dark:bg-white/5 p-1.5 rounded-2xl border border-slate-100 dark:border-white/5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <AnimatePresence mode="popLayout">
            {filteredShipments.length === 0 ? (
              <motion.div 
                key="empty"
                variants={itemVariants}
                className="bg-white dark:bg-slate-900 p-16 rounded-[3rem] border border-slate-100 dark:border-slate-800 text-center space-y-6"
              >
              ...
              filteredShipments.map((shipment, index) => (
                <motion.div
                  ...
                  variants={itemVariants}
                  whileHover={{ y: -5 }}
                  className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl hover:border-primary/20 transition-all group"
                >
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 mb-8">
                    <div className="flex items-center gap-4">
                      <div className="w-16 h-16 bg-slate-50 dark:bg-white/5 rounded-3xl flex items-center justify-center text-slate-400 group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                    ...
                    <div className="text-left sm:text-right space-y-1">
                      ...
                      <p className="text-xs font-bold text-slate-500 dark:text-slate-400 truncate max-w-full">
                  ...
                  <div className="relative py-8">
                    <div className="absolute top-[50px] left-0 w-full h-[6px] bg-slate-100 dark:bg-white/5 rounded-full" />
                    <motion.div 
                        ...
                        return (
                          <div key={step.status} className="flex flex-col items-center gap-3">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center z-20 transition-all duration-500 ${
                              isActive 
                                ? 'bg-primary text-white shadow-xl shadow-primary/30 ring-4 ring-white dark:ring-slate-900 scale-110' 
                            ...
                            <div className="text-center">
                              <span className={`text-[10px] font-black uppercase tracking-widest block transition-colors ${
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-8 pt-8 border-t border-slate-50 dark:border-white/5">
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <p className="font-bold text-xs dark:text-white">KamerFresh Logistics</p>
                      </div>
                    </div>
                    <div className="space-y-1">
                    ...
                    <div className="flex items-center justify-end">
                      <button className="w-full sm:w-auto bg-primary/10 hover:bg-primary text-primary hover:text-white px-6 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all">
                        {t('details')}
                      </button>
                    </div>
                  </div>
        ...
        <div className="space-y-8">
          {/* Map Card */}
          <motion.div variants={itemVariants} className="bg-slate-950 text-white p-8 rounded-[3rem] shadow-2xl relative overflow-hidden group">
            <div className="absolute -right-20 -top-20 w-64 h-64 bg-primary/20 rounded-full blur-[80px] pointer-events-none group-hover:bg-primary/30 transition-all duration-700 font-black"></div>
            <div className="relative z-10 space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-xl font-black tracking-tight">{t('live_tracking')}</h3>
                <span className="bg-primary/20 text-primary-light px-2 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest animate-pulse">Live Tracking</span>
              </div>
              ...
              <div className="aspect-square bg-slate-900 rounded-[2rem] border border-white/10 flex flex-col items-center justify-center relative group-hover:scale-[1.02] transition-transform duration-500 overflow-hidden">
                ...
                />
                <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-slate-950 to-transparent z-[20] pointer-events-none">
                  <div className="flex items-center gap-3">
                    <div className="w-2 h-2 bg-primary rounded-full animate-pulse" />
                    <p className="text-[10px] font-black tracking-[0.2em] uppercase text-white/90 font-mono italic">GPS ACTIVE: {user?.location_name || 'Yaoundé, CM'}</p>
                  </div>
                </div>
              </div>
          ...
          {/* Quick Stats Card */}
          <motion.div variants={itemVariants} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-slate-800 shadow-sm space-y-8">
              ...
              {[
                ...
                <div key={i} className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic">{t(stat.label)}</span>
                  </div>
            ...
            <div className="pt-6 border-t border-slate-50 dark:border-white/5">
              <div className="flex items-center justify-between text-[10px] font-black uppercase tracking-widest text-slate-400 mb-4 italic">
                <span>{t('monthly_summary')}</span>
                <span className="font-mono">{stats.total} {t('items')}</span>
              </div>
              <div className="h-2.5 bg-slate-100 dark:bg-white/5 rounded-full overflow-hidden flex">
                <div className="h-full bg-primary transition-all duration-1000" style={{ width: `${stats.total ? (stats.active / stats.total) * 100 : 0}%` }} />
                <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${stats.total ? (stats.completed / stats.total) * 100 : 0}%` }} />
              </div>
              <div className="flex gap-4 mt-4">
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-primary" />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('active')}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <div className="w-2 h-2 rounded-full bg-emerald-500" />
                  <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{t('completed')}</span>
                </div>
              </div>
            </div>
          </motion.div>
