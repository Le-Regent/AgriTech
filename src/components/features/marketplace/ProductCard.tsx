...
export default function ProductCard({ 
  ...
  return (
    ...
    >
      <div className="aspect-[3/4] sm:aspect-square lg:aspect-[4/5] relative overflow-hidden">
        ...
        {showBadges && (
          <div className="absolute top-3 left-3 sm:top-4 sm:left-4 flex flex-col gap-1.5 sm:gap-2">
            {product.is_verified && (
              <div className="bg-white/90 dark:bg-surface-dark/90 backdrop-blur px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm flex items-center gap-1 w-fit">
                <span className="material-symbols-outlined text-primary text-[12px] sm:text-[14px] fill-1">verified</span>
                <span className="text-[8px] sm:text-[10px] font-black uppercase tracking-widest text-slate-900 dark:text-white">Verified</span>
              </div>
            )}
            {product.health_status && product.health_status !== 'N/A' && (
              <div className={`backdrop-blur px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm flex items-center gap-1 text-white w-fit ${
                product.health_status === 'Critical' ? 'bg-red-600/90 animate-pulse' : 
                product.health_status === 'Warning' ? 'bg-amber-500/90' : 
                'bg-primary/90'
              }`}>
                <span className="material-symbols-outlined text-[12px] sm:text-[14px] fill-1">
            ...
            <div className="bg-emerald-500/90 backdrop-blur px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-lg shadow-sm flex items-center gap-1 text-white w-fit">
              <span className="material-symbols-outlined text-[12px] sm:text-[14px] fill-1">inventory</span>
      ...
      <div className="p-3 sm:p-5 space-y-2 sm:space-y-4 flex-1 flex flex-col min-h-[115px] sm:min-h-[140px]">
        <div className="flex-1 min-w-0">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between mb-1 gap-1">
            <h3 className="font-black text-[13px] sm:text-lg group-hover:text-primary dark:text-white transition-colors truncate pr-1" title={product.name}>
              {product.name}
            </h3>
          ...
          <div className="flex flex-col gap-1.5 mt-1 sm:mt-2">
            ...
            >
              ...
              <span className="hidden sm:block text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-slate-500 dark:text-slate-400 truncate">
                {product.category}
              </span>
            </div>
          </div>
